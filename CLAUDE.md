...
- **Link verification rule (2026-05-01 — sitemap incident):** When building or editing any HTML page that contains links, NEVER verify links by checking raw URL paths with curl from the server root. ALWAYS verify by resolving links the way a browser would from the page's actual location. A link `href="/foo.html"` on a page served at `/view/sitemap.html` resolves to `http://server/foo.html` (root) — NOT `http://server/view/foo.html`. Use `el.href` in Playwright (or equivalent browser-resolved URL) to get the true destination. The `sitemap_links_navigable` Playwright test in `visual_overlay_agent.py` enforces this automatically for the sitemap page on every guardrail run.

- Ridgecrest 360 app: BUILT AND RUNNING — standalone 360° room documentation PWA at /home/claudeuser/sphere_docs/ — HTTP port 8091, HTTPS port 8444 (self-signed cert, mobile camera works). Tech stack: Flask, OpenCV 4.13 stitcher, Pannellum.js sphere viewer, SQLite DB. Start: `cd ~/sphere_docs && python3 app.py --https`. Planned subdomain 360.ridgecrestdesigns.com (DNS not yet set — nginx snippet in sphere_docs/CLAUDE.md). No shared code with main site. See sphere_docs/CLAUDE.md for full governance.

- Henry can run sudo commands from the DigitalOcean Console terminal (no ! prefix needed there). Use this approach when Claude cannot invoke sudo — e.g., to open firewall ports (ufw allow), install packages, or run systemctl. Confirmed 2026-05-02: Henry ran 'ufw allow 8444/tcp' from the DO Console to open the Ridgecrest 360 HTTPS port.
