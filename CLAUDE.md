# Ridgecrest Designs — Strategy & Rules
...
## 24. Active Rules Derived from Past Work

> Full history in `CLAUDE_HISTORY.md`. Only forward-looking rules live here.
...
1. **`set-version` gap:** Does NOT update static non-portfolio pages (`index.html`, `portfolio.html`, `about.html`, `contact.html`, `process.html`, `team.html`) — planned fix in `preview_server.py`, not yet implemented
...
### Agent Conduct Rules (from `ridgecrest-agency/rules/AGENT_RULES.md`)
...
- **Rule 13 — No Stale Tasks:** Session summaries are context, not a work queue — only one atomic carry-over action per session start, then stop and wait
- **Rule 14 — Single Source of Truth:** Every element has exactly one owner (DB, JS injection, or static HTML) — check what already controls an element before adding new logic
