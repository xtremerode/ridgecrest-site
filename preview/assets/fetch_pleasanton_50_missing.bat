...
echo Fetching 50 missing Pleasanton Custom images (tries jpg/png/webp)...
echo.
...
set TMPDIR=%TEMP%\pleasanton_50_missing
set OK=0
set FAIL=0
...
set HDRS=-H "Referer: https://www.ridgecrestdesigns.com/" -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"

REM Try .jpg
set TMP=%TMPDIR%\%HASH%_mv2.jpg
curl.exe -s -L %HDRS% -o "%TMP%" "https://static.wixstatic.com/media/%HASH%~mv2.jpg"
for %%A in ("%TMP%") do set SZ=%%~zA
if defined SZ if !SZ! GTR 10000 goto :doupload

REM Try .png
set TMP=%TMPDIR%\%HASH%_mv2.png
curl.exe -s -L %HDRS% -o "%TMP%" "https://static.wixstatic.com/media/%HASH%~mv2.png"
for %%A in ("%TMP%") do set SZ=%%~zA
if defined SZ if !SZ! GTR 10000 goto :doupload

REM Try .webp
set TMP=%TMPDIR%\%HASH%_mv2.webp
curl.exe -s -L %HDRS% -o "%TMP%" "https://static.wixstatic.com/media/%HASH%~mv2.webp"
for %%A in ("%TMP%") do set SZ=%%~zA
if defined SZ if !SZ! GTR 10000 goto :doupload

echo   FAILED: could not download in any format
set /a FAIL+=1
goto :eof

:doupload
curl.exe -s -X POST "%UPLOAD_URL%" ^
  -H "X-Admin-Token: %TOKEN%" ^
  -F "file=@%TMP%;filename=%HASH%_mv2.jpg;type=image/jpeg"
echo   OK (%SZ% bytes)
set /a OK+=1
goto :eof
