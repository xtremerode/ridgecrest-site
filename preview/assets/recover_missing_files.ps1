# recover_missing_files.ps1
# Re-uploads 33 images whose files are missing from disk (already in gallery DB but file was deleted).
# The server now saves the file even when already in gallery, if the file is missing on disk.
...
$TempDir = Join-Path $env:TEMP "rdrecover2_$([System.Guid]::NewGuid().ToString('N').Substring(0,8))"
New-Item -ItemType Directory -Path $TempDir -Force | Out-Null

Write-Host ""
Write-Host "=== Ridgecrest File Recovery (Round 2) ===" -ForegroundColor Cyan
Write-Host "Re-uploading $($images.Count) images with missing files..."
Write-Host ""
...
Write-Host ""
Write-Host "Downloaded $($ready.Count)/$($images.Count). Uploading..." -ForegroundColor Yellow
Write-Host ""

$LF = "`r`n"; $added = 0

foreach ($entry in $ready) {
    $img=$entry.img; $hash=$img.hash; $ext=$img.ext; $slug=$img.slug
    ...
    $hdrStr  = "--$boundary$LF" + "Content-Disposition: form-data; name=`"file`"; filename=`"$hash.$ext`"$LF" + "Content-Type: $mimeType$LF$LF"
    ...
    try {
        ...
        $req.Method="POST"; $req.ContentType="multipart/form-data; boundary=$boundary"
        $req.ContentLength=$body.Length; $req.Headers.Add("X-Admin-Token",$TOKEN)
        ...
        if ($obj.ok) { Write-Host "OK" -ForegroundColor Green; $added++ }
        else { Write-Host "FAILED - $($obj.error)" -ForegroundColor Red }
    } catch { Write-Host "FAILED - $($_.Exception.Message)" -ForegroundColor Red }
}

Remove-Item $TempDir -Recurse -Force -ErrorAction SilentlyContinue
Write-Host ""
Write-Host "=== Done. Saved $added/$($ready.Count) files to server. ===" -ForegroundColor Cyan
if ($failed.Count -gt 0) { Write-Host "$($failed.Count) still blocked by Wix." -ForegroundColor Yellow }
Write-Host ""
