...
- Surgical mode modal expansion (2026-05-03): When surgical mode is ON in the rerender modal (`toggleSurgicalMode()` in `pages.html`), the modal expands to `max-width:1400px; width:96vw` with panel grid `2fr 1fr` (source gets ~2/3 of modal for precise watermark selection). OFF restores to `max-width:860px; width:95vw` with `1fr 1fr`. `openRerender()` always resets to normal size — never leaves modal expanded after close/reopen. `rerenderPanelGrid` is the stable DOM id for grid control. Do not change these dimensions without a Playwright test verifying ON→expanded, OFF→restored, and reopen→normal.
...
- Crop mode in admin pages.html (2026-05-04): The rerender modal has a crop mode toggled by toggleCropMode(). When ON: modal expands to max-width:1400px (same as surgical mode), canvas shown, contain-mode coordinate math used (full image visible, no cover clipping). 800px minimum width enforced only in crop mode; surgical mode unaffected. Crop mode is mutually exclusive with surgical mode — enabling one disables the other. _cropModeOff() hides canvas and resets the Crop button only when surgical mode is also off. applyCrop() calls POST /admin/api/images/crop, populates the result panel, sets _rerenderResultPath, shows Use/Restart buttons, calls _stripAppend, and auto-exits crop mode. Server endpoint: PIL crop from native coordinates, downscale-only (never upscale) variants (_960w, _1920w, _480w), records crop in image_crop_history table, returns display_path/hero_path/crop_native_w/crop_native_h. cropMouseDown/Move/Up fire in both surgical and crop modes. Playwright test crop_mode_toggle verifies ON/OFF expansion, surgical mutual exclusion, and full apply-crop flow; cleans up disk files and DB record in finally block.

- Diagnostic-first rule for third-party iframe issues (learned 2026-05-04): When a postMessage/iframe resize problem is reported, run a CDP/Playwright diagnostic to capture actual messages BEFORE touching any code. Theorizing from source code without measuring actual behavior led to two wasted guardrail cycles on start-a-project.html. The Base44 postMessage diagnostic (intercept all messages, check heights at each step) takes 2 minutes to write and immediately reveals the root cause. Code-first implementation without measurement is forbidden for third-party iframe behavior.

- start-a-project.html Base44 iframe architecture (2026-05-04): Base44's ResizeObserver is orphaned between wizard steps. Their #root > firstElementChild is a NEW DOM node for each step — the observer set up on landing screen t is destroyed when navigating. The route-change useEffect only fires landing→Step1 (URL change); Step1→Step2 is pure React local state with no route change and no resize message, ever. Step 2 content is ~1029px but Base44 sends ZERO postMessages for it. Current fix: MIN_H=1080, MAX_H=2400 in start-a-project.html — Step 2 fits without scrollbar. Trade-off: Steps 0/1 have ~350-450px whitespace below shorter content. Pending: proxy fix (MutationObserver on iframe internals to inject synthetic resize messages) was proposed but NOT implemented as of 2026-05-04.
